...
architecture Behavioral of reflex_game is
    ...
    -- UART TX message buffer (max 16 bytes)
...
begin
    ...
    ----------------------------------------------------------------------------
    -- Game process: FSM + debounce + LFSR + UART message build
    ----------------------------------------------------------------------------
    ...
    begin
        if rising_edge(clk) then
            ...
            if tick_1ms_now = '1' then
                ...
                -- Normalize button to active-high internal representation
                ...
                -- Debounce: require DEBOUNCE_MS stable ms before accepting change
                ...
                case state is
                    ...
                    when LED_ON =>
                        if btn_rise_now = '1' then
                            hold_ms <= 0;
                            -- Build "RESULT_MS=NNNN\r\n" (16 bytes)
                            -- BCD conversion via subtraction (no division, XST-safe)
    ...
    led_reflexo <= '1' when state = LED_ON else
                   '1' when state = RESULT_OK and reaction_time_ms >= 250 else
                   '0';

    led_status <= '1' when state = IDLE else
                  '1' when state = RESULT_OK else
                  '0';

    led_erro <= '1' when state = EARLY_PRESS else
                '1' when state = TIMEOUT else
                '1' when state = RESULT_OK and reaction_time_ms >= 500 else
                '0';

    ----------------------------------------------------------------------------
    -- FT2232H Async FIFO TX (245-FIFO mode)
    -- uart_trigger starts transmission of uart_msg[0..uart_msg_len-1]
    -- usb_txe='0' -> FT2232H TX FIFO not full, can write
    -- usb_wr='0'  -> active-low write strobe (hold 2+ cycles = 50ns at 40MHz)
    ----------------------------------------------------------------------------
